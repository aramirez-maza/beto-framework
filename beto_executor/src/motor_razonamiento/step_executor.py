...
PROMPTS_POR_PASO: dict[int, str] = {
    ...
    6: (
        "Eres un asistente de cierre BETO Framework v4.3 — Operational Semantic Closure Layer. "
        "Usando todos los BETO_COREs hijos, genera el CIERRE_ASISTIDO_OPERATIVO.md. "
        "Este paso es CIERRE_ASISTIDO_OPERATIVO (no CIERRE_ASISTIDO simple): "
        "su propósito es promover OQs críticas a estados ejecutables. "
        ""
        "PROCESO OBLIGATORIO: "
        "1. Para cada OQ crítica (oq_type en OQ_POLICY, OQ_EXECUTION, OQ_EXCEPTION, OQ_DATA_SEMANTICS): "
        "   a. Ejecutar EXECUTION_READINESS_CHECK con los 8 campos: "
        "      alcance, trigger, input, output, constraint, fallback, exception, trazabilidad. "
        "   b. Detectar patrones blandos: alto, bajo, adecuado, estándar, rápido, importante, "
        "      cuando sea necesario, según convenga, si aplica. "
        "      → no invalidan la respuesta, pero exigen validación operativa. "
        "   c. Asignar resultado: PASS_EXECUTABLE → DECLARED_EXECUTABLE | "
        "      PASS_WITH_LIMITS → DECLARED_WITH_LIMITS | "
        "      FAIL_EXECUTIONAL_GAP → DECLARED_RAW + BETO_GAP_EXECUTIONAL. "
        "   d. Si FAIL: realizar hasta max_operational_requestions=2 repreguntas tipadas "
        "      para obtener especificación ejecutable. Si después de 2 repreguntas sigue "
        "      en FAIL: registrar como DECLARED_RAW + BETO_GAP_EXECUTIONAL activo. "
        ""
        "2. Generar sección 'Execution Summary' con estados finales por OQ. "
        "3. Generar sección 'Gate G-2B — Operational Readiness Gate' con resultado: "
        "   APPROVED_EXECUTABLE | APPROVED_WITH_LIMITS | BLOCKED_BY_EXECUTIONAL_GAPS. "
        "4. Actualizar todos los BETO_COREs a SUCCESS_CLOSED. "
        "5. Para OQs no críticas: cerrar con modo BETO_ASSISTED como en versiones anteriores. "
        ""
        "COMPATIBILIDAD BETO_PARALELO: el cierre operativo es LOCAL a cada unidad. "
        "Cada unidad mantiene OQs propias, execution_state propio, gaps propios. "
        "Una unidad BLOCKED no bloquea globalmente. "
        "Gate G-2B se evalúa POR UNIDAD en sistemas con nodos PARALLEL. "
        ""
        "ARTEFACTOS A GENERAR: "
        "- CIERRE_ASISTIDO_OPERATIVO.md (artefacto principal) "
        "- EXECUTIONAL_GAP_REGISTRY.md (si hay BETO_GAP_EXECUTIONAL activos) "
